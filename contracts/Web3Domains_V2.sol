...
contract Web3Domains is ERC721, ERC721Enumerable, AdminControl, RecordStorage, WhiteList, BookingList
{
	...
	mapping(string => uint256) public _tokenIdMaps;
    ...
    mapping(bytes => string) private _tlds;
	...
	constructor() ERC721("Wanchain Web3 Domains", "WW3D") {
	...
	function getOwner(string memory name) public view returns (address)  {
		string memory _name = StringUtil.toLower(name);
	    uint256 tokenId = _tokenIdMaps[_name];
	...
	function setTLD(string memory _tld) public onlyOwner {
		bytes memory bytetld = bytes(_tld);
		_tlds[bytetld] = _tld;
    }
	...
	function isTLD(string memory _tld) public view returns (bool) {
		bool isExist = false;
        bytes memory bytetld = bytes(_tld);
		if (bytes(_tlds[bytetld]).length != 0){
            isExist = true;
        }
		return isExist;
	}
	...
	function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory _domain = _tokenURIs[tokenId];

        // If there is no base URI, return the domain.
		string memory baseURI = _baseURI();
        ...
        if (bytes(_domain).length > 0) {
            return string(abi.encodePacked(baseURI, _domain));
        }
	...
	function _setTokenURI(uint256 tokenId, string memory _tokenURI) internal virtual {
		...
		_tokenIdMaps[_tokenURI] = tokenId;
    }
	
	function getTokenId(string memory name) public view returns (uint256)
	{
		return _tokenIdMaps[name];
    }
	
	
	/**
     * Begin: Register, Buy, Transfer
     */
	function buyDomain(address to, string memory domain, string memory tld) external payable 
	{
	    ...
	    uint256 tokenIdchk = _tokenIdMaps[_domain];
		
		require (!_exists(tokenIdchk), "Domain already exists");
	...
	function registerDomainBatch(address[] calldata owners, string[] memory domains, string memory tld) external onlyMinterController 
	{
		for (uint256 i = 0; i < owners.length; i++) 
		{
			...
			{
				uint256 tokenIdchk = _tokenIdMaps[domain];
				if (!_exists(tokenIdchk))
				{
	...
	function burn(uint256 tokenId) public virtual {
		...
		if (bytes(_tokenURIs[tokenId]).length != 0) {
			...
			delete _tokenIdMaps[_tokenURIs[tokenId]];
        }
	...
	function _safeMintSubDomain(address to, uint256 tokenId, string memory sub, bytes memory _data) internal {
        ...
        string memory _subdomain = string(abi.encodePacked(_sub, ".", _domain));
		
		uint256 tokenIdchk = _tokenIdMaps[_subdomain];
		
		require (!_exists(tokenIdchk), "Domain already exists");
		
		uint256 _newTokenId = _tokenIdTracker.current();
        ...
        _setTokenURI(_newTokenId, _subdomain);
		
		_tokenIdTracker.increment();

        emit NewURI(_newTokenId, _subdomain);
    }
	...
	function _burnSubDomain(uint256 tokenId, string memory sub) internal {
		...
		uint256 _subTokenId = _tokenIdMaps[string(_newUri)];
		
        // remove sub tokenIds itself
        _subTokens[tokenId].remove(_subTokenId);
		
		if (bytes(_tokenURIs[_subTokenId]).length != 0) {
            delete _tokenURIs[_subTokenId];
			delete _tokenIdMaps[_tokenURIs[_subTokenId]];
        }
